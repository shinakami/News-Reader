...
股票 Dashboard 的自訂背景圖片功能使用 Pillow；建立或更新虛擬環境後請安裝需求檔：
...
主視窗右上角的「背景設定」可載入 PNG、JPG、JPEG、BMP、GIF 或 WEBP 圖片，並以滑桿調整 0%～100% 圖片透明度。圖片使用全域座標連續鋪設於標題列、指數卡片、表格周圍與 K 線圖，並同步套用到台股、美股及已開啟的台指期貨視窗；表格儲存格與操作按鈕保留深色底以維持行情可讀性。圖片配置支援 Windows 常見的「填滿、符合、延伸、並排、置中」五種模式；0% 會完整顯示圖片，100% 會完全隱藏圖片，也可按「清除背景」恢復原本的深夜底色。背景只在本次程式執行期間使用，不會複製圖片或將圖片加入版控。
