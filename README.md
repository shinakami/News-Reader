...
主視窗與 ETF 詳細視窗會響應視窗縮放：指數卡片會在 4 欄、2 欄、1 欄之間切換，圖表與 ETF 表格也會在寬視窗左右排列、窄視窗上下排列。
四大指數曲線圖以 2x2 區塊呈現，每個指數使用獨立座標範圍，並採用類似 Yahoo 股市的盤中線圖樣式：昨收基準線、漲紅跌綠、右側價格軸與線下淡色填滿。圖表區、ETF 表格區，以及四個指數圖表區塊之間都有可拖曳分隔線，可手動調整大小。
